...
# End command 4: remove managed teams from org using teams config.

ORG="${1:-}"
[[ -n "$ORG" ]] || { echo "Usage: bash scripts/end/remove_org_teams.sh <org>" >&2; exit 1; }
...
cmd=(bash "$ROOT_DIR/scripts/remove_teams_org.sh" --config "$teams_cfg" --org "$ORG")
